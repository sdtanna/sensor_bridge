...
def parseStandardFrame(frameData):
    ...
    outputDict = {'error': 0, 'trackData': None, 'numTargets': 0}

    # Read in frame Header
    try:
        header = struct.unpack(headerStruct, frameData[:frameHeaderLen])
        magic, version, totalPacketLen, platform, frameNum, *rest = header
    ...
    numTLVs = rest[2]  # rest[2] is numTLVs

    for _ in range(numTLVs):
        try:
            tlvType, tlvLength = tlvHeaderDecode(frameData[:tlvHeaderLength])
            tlvData = frameData[tlvHeaderLength:tlvHeaderLength+tlvLength]

            # Check if TLV is for track data
            if tlvType in [MMWDEMO_OUTPUT_MSG_TRACKERPROC_3D_TARGET_LIST, MMWDEMO_OUTPUT_EXT_MSG_TARGET_LIST]:
                numTargets, trackData = parseTrackTLV(tlvData, tlvLength)
                outputDict['trackData'] = trackData
                outputDict['numTargets'] = numTargets
                break  # Break the loop as we found the track data
...
#     except:
#         logger.error('Error: Failed to initialize pointCloud array')
#         outputDict['error'] = 1
...
#         except:
#             logger.error('TLV Header Parsing Failure: Ignored frame due to parsing error')
...
#         logger.error('Warning: Frame packet length read is not equal to totalPacketLen in frame header. Subsequent frames may be dropped.')
#         outputDict['error'] = 3
